...
function stepsTaken(distances, point, remains) {
	
	var stepsGroup = distances.slice(0, point+1);
	var sumSteps = 0;
	for (a = 0; a < stepsGroup.length; a++){
		
		sumSteps += stepsGroup[a] 
		
	}

	return sumSteps + 1 - (distances[point] - remains);
...
for (i = 0; i < firstWireAmount.length; i++){
	...
	if (firstWireDirection[i] == "right"){
		...
		for (d = 0; d < firstWireAmount[i]; d++){
			...
			firstWirePath[firstWirePos.x + "," + firstWirePos.y] = stepsTaken(firstWireAmount, i, d);
...
for (i = 0; i < secondWireAmount.length; i++){
	...
	if (secondWireDirection[i] == "right"){
		...
		for (d = 0; d < secondWireAmount[i]; d++){
			...
			secondWirePath[secondWirePos.x + "," + secondWirePos.y] = stepsTaken(secondWireAmount, i, d);
...
for (i = 0; i < firstWireKeys.length; i++){
	...
	//compare values from first wire array to keys in second wire object, if a key exists along secondWirePath it will return i instead of undefined
	if (secondWirePath[firstWireKeys[i]] != undefined){
		
		//add matching keys to get number of steps
		crossedWires.push(firstWirePath[firstWireKeys[i]] + secondWirePath[firstWireKeys[i]]);
...
document.getElementById("result3b").innerHTML = "The lowest amount of steps taken from the central port to the closest intersection is " + crossedWires[0] + "."; 